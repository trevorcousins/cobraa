...
plt.stairs(edges=(time_array/mu)*gen,values=(1/final_params[:,2])/mu,label='$N_A(t)$',linewidth=4,linestyle="solid",baseline=None)
plt.stairs(edges=(time_array/mu)*gen,values=(1/final_params[ts:te+1,3])/mu,label='$N_B(t)$',linewidth=4,linestyle="solid",baseline=None)
plt.xscale('log')
plt.ylabel('$N_e(t)$')
plt.xlabel('Years')
```   
...
N_B = (1/inference[,4])/mu
plot(time,N_A,log="x",type="n",xlab="Years",ylab="Time")
plot(time,N_B,log="x",type="n",xlab="Years",ylab="Time")
lines(time, N_A, type="s", col="red")
lines(time, N_B, type="s", col="blue")
...
You can decode the HMM (that is, get the inferred coalescence times and admixture regions across the genome) with: 

```python /path/to/installation/cobraa/cobraa.py -in <infile> -D <D> -o <outprefix> -decode -decode_downsample <decode_downsize> | tee <logfile.txt>```
...
If you are inferring parameters, and you set `-o /path/to/installation/cobraa_inference/structure_` then the file describing the inferred parameters will be saved to `/path/to/installation/PSMC+cobraa_inference/structure_final_parameters`. Default behaviour is to save to the current working directory. 
...
```python /path/to/installation/cobraa/simulate_HMM.py -D 10 -theta 0.001 -rho 0.0005 -o_mhs simulations/sim1_variants.mhs -o_coal simulations/sim1_coal.txt.gz -spread_1 0.1 -spread_2 50 -L 1000000 -gamma_``` TODO
