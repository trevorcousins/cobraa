...
# script written by Aylwyn Scally, I believe
...
class MaskIterator:
    ...
    def readLine(self):
        try:
            ...
            # pdb.set_trace()
            self.eof = True
            # print('setting eof True',file=sys.stderr)
    ...
    def getVal(self, pos):
        ...
        while not self.eof and pos > self.end:
            self.readLine()
        if self.eof:
            # return None # original by Aylwyn Scally
            return False if not self.negative else True # updated by Trevor Cousins tc557
        if pos >= self.start and pos <= self.end:
            return True if not self.negative else False # I think True means "this pos is callable " 
        else:
...
class MergedMask:
    ...
    def getVal(self, pos):
        # if pos % 10000:    
        #     for m in self.maskIterators:
        #         print(f'm.file is {m.file}; m.getVal(pos) is {m.getVal(pos)}',file=sys.stderr)
...
class VcfIterator:
    ...
    def __next__(self):
        ...
        if len(geno) != 3 :
                ...
                phased = geno[1] == "|"
        # print(f'int(geno[0]),int(geno[2])={int(geno[0]),int(geno[2])}')
        try:
            check = int(geno[0])
            check = int(geno[2])
        except:
            print(f'problem with converting genotype to int: (geno[0],geno[1])={geno[0],geno[2]}; ')
